...
/**
 * Created by timbauer on 7/29/15.
 */
public class DraftScreenPickBanAdapter extends BaseAdapter {

    Context mContext;
    int maxNumOfChamps;
    int numOfChamps = 0;

    public DraftScreenPickBanAdapter(Context context, int maxNumOfChamps){
        mContext = context;
        this.maxNumOfChamps = maxNumOfChamps;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
         ...
         imageView.setImageResource(selectedChampImages.get(position));
         return imageView;
    }

    public void addChamp(Integer champion){
        selectedChampImages.add(champion);
        numOfChamps++;
        this.notifyDataSetChanged();
    }

    private ArrayList<Integer> selectedChampImages = new ArrayList<Integer>(maxNumOfChamps);
